...
new_key = np.memmap(os.path.join(args.save_dir,
        f'dstore_merge{args.k}_size{num}_embed{args.dimension}_keys.npy'), mode='w+', dtype=np.float16, shape=(num, args.dimension))
new_val = np.memmap(os.path.join(args.save_dir,
        f'dstore_merge{args.k}_size{num}_embed{args.dimension}_vals.npy'), mode='w+', dtype=np.int, shape=(num, 1))
new_weight = np.memmap(os.path.join(args.save_dir,
        f'dstore_merge{args.k}_size{num}_embed{args.dimension}_weights.npy'), mode='w+', dtype=np.int, shape=(num, 1))
